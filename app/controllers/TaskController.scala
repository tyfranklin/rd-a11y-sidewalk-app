          ...
          for (label: LabelSubmission <- data.labels) {
            ...
            val labelId: Int = existingLabelId match {
              ...
              case None =>
                ...
                //changed so that the older version of my database would work
                LabelTable.save(Label(0, auditTaskId, label.gsvPanoramaId, labelTypeId,
                                      label.photographerHeading, label.photographerPitch, label.panoramaLat,
                                      label.panoramaLng, label.deleted.value, label.temporaryLabelId))
            }
