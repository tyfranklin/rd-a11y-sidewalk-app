...
object Populator {
  ...
  def populateLabelStreetTable(): Unit = {
    ...
    val sampleIndexes = getSampleEdges(seed, max)
      ...
      var allLabels: Array[(Int, Int, Int)] = new Array[(Int, Int, Int)](0)
      for(sampleIndex: Int <- sampleIndexes){
          ...
          for(i <- 0 until path.length){
              ...
              logprint("newLabels count: " + newLabels.size);
      ...
      logprint("number of labels made: " + allLabels.size);
      //LabelStreetTable()
      for(labelData : (Int,Int,Int) <- allLabels) {
        logprint(labelData._1 + ", " + labelData._2 + ", " + labelData._3)
  ...
  def getSampleEdges(seed: Int, max: Int): Array[Int] ={
    ...
    var edges: Array[Int] = new Array[Int](numSamples)
    logprint(edges.size + "");
